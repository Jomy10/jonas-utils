...
public class ProgressBarHandler implements be.jonaseveraert.util.progressBar.ProgressBarHandler {
    ...
    /**
     * <p>Creates a handler that can handle a {@code JProgressBar} in a {@code JPanel}</p>
     * <h2>Usage</h2>
     * <h3>setup</h3>
     * Call the {@link #setNumSubProcesses(int) setNumbProcesses} method to set the amount of sub processes that need to
     * be completed. This will divide the progress bar in {@link #numSubProcesses numSubProcesses} equal parts.<p>
     ...
     * This is used to divide the part of the progressbar that the sub-process takes up into {@code numActivities} equal parts.<p>
     * Sub-processes also need a name and state. The default state is {@link #SUBPROCESSINFO_BUSY busy}, but it can also
     * be set to {@link #SUBPROCESSINFO_DONE done}, which will place a "." after the sub-process instead of a "...".</p>
     *
     ...
     * It will also set the cursor to {@link #CURSOR_BUSY busy}.
     * <p>To get the current {@link #progress progress percentage}, call the {@link #getProgress() getProgress} method. This can
     * be use to determine if the progressbar has finished externally.</p>
     * <p>To complete an activity, call the {@link #completeActivity(boolean) completeActivity(boolean autoCompleteSubProcess)}. If the method's
     * boolean value is set to true, it will automatically go to the next sub-process after completing all activities in the sub-process.</p>
     * <p>If you want to go to the next sub-process (e.g. not all activities had to be performed, or a sub-process had to be skipped),
     ...
     * manage other things that have to be done after the progress bar has hit 100% (or if the process has been cancelled).</p>
